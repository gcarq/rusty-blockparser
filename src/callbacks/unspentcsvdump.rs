...
mod tests {
    ...
    fn test_callback() {
        let matches =
            UnspentCsvDump::build_subcommand().get_matches_from(vec!["unspentcsvdump", "/tmp"]);
        let mut callback = UnspentCsvDump::new(&matches).unwrap();

        callback.on_start(&CoinType::from(Bitcoin), 0);
        ...
        assert_eq!(
            utils::arr_to_hex_swapped(&block1.txs[0].hash),
            "fff2525b8931402dd09222c50775608f75787bd2b87e56995a7bdd30f79702c4"
        );
        assert_eq!(block1.txs[0].value.outputs[0].out.value, 556000000);

        callback.on_block(&block1, 100000);
        let value = callback
            .unspents
            .get(&TxOutpoint::new(block1.txs[0].hash, 0).to_bytes())
            .unwrap();
        assert_eq!(value.block_height, 100000);
        assert_eq!(value.output_val, 556000000);
        ...
        assert_eq!(
            utils::arr_to_hex_swapped(&block2.txs[0].hash),
            "5aa8e36f9423ee5fcf17c1d0d45d6988b8a5773eae8ad25d945bf34352040009"
        );
        assert_eq!(block2.txs[0].value.outputs[0].out.value, 9070000000);

        callback.on_block(&block2, 105001);
        ...
        assert_eq!(value.output_val, 9070000000);
